	...
	while($row = $result->fetch_assoc()){
		...
		if($btnUpdateClicked == true) {
			...
			if($newStatusId == 4) { //Arrived
				$sql ="UPDATE `dispatch` SET `time_arrived`=now() WHERE time_arrived is null and patorlcar_id='" . $carId . "'";
			$updateSuccess = $conn->query($sql);
			...
			$sql ="SELECT incident_id FROM `dispatch` WHERE time_completed is null and patorlcar_id='" . $carId . "'";
			$result = $conn->query($sql);
			$incidentId = 0;
			...
			$sql ="UPDATE `dispatch` SET `time_completed`=3 WHERE incident_id='" . $incidentId . "'";
			$updateSuccess = $conn->query($sql);
		
			if($updateSuccess == false) {
				echo "Error:" . $sql . "<br>" . $conn->error;
			}
			$sql ="UPDATE `incident` SET `incident_status`=now() WHERE time_completed is null and patorlcar_id='" . $carId . "'";
			$updateSuccess = $conn->query($sql);
